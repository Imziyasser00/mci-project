...
if(isset($_SESSION['nom']) and (isset($_POST['idproc']) || isset($_GET['idproc']))){
	...
	$correction=$connexion->exec("INSERT INTO planaction VALUES('',$idproc,$ann,0,0)");	
	
	$creation=$connexion->query("select * from  nonconfirmitecreation where id_proc=".$idproc." and ann=".$ann."");
	$verif=$creation->rowcount();
	if($verif==0){
	
		$correction=$connexion->exec("INSERT INTO nonconfirmitecreation VALUES('',$idproc,$ann)");
	$co=$connexion->query('select * from point where type!="ref" and type!="champ"  and type!="fort" ');
							...
							$c=$selectNumero->rowcount();
								...
								$numero=$numeros['numero'];
										...
										<?php } 
											...
											?>
										<input onclick="location.href = '#'" class="btn green" type="button" value="Plan confirmé" disabled="disabled">
										<?php
										}
