														...
														$audprenom=$sel['prenom'];
															$selutilisateur=$connexion->query("select id , mail from utilisateur where nom='".$audnom."' and prenom='".$audprenom."'");
																$selid = $selutilisateur->Fetch(PDO::FETCH_ASSOC);
																	$i_ra=$selid['id'];
																	$mail=$selid['mail'];
																		$addFiche=$connexion->exec("INSERT INTO fichecreeoupas VALUES ('',$ann,'$numero',0,$i_ra)");
								$to = $mail;
								$subject = "Audit ".$numero;


								$txt = 'Vous êtes responsable d\'audit n° '.$numero.' du processus '.$ssnom.' <br>veuillez <a href="http://192.168.2.106/SMI/Mci_QSE/admin/login.php">vous connecter</a> pour renseigner la fiche d\'audit.
