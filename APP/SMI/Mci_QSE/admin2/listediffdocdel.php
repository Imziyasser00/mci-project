...
if (version_compare(PHP_VERSION, '5.4.0', '>=')) {
  ...
  ob_start(null, 0, false);
	...
	$servnames=$connexion->query("select nom from service where service.id=$r10");
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-file-o"></i>Liste des diffusions
							</div>
						...
						<div class="portlet-body">
							...
							<tbody>
								...
								if($adp==2){
								$req1=$connexion->query('select diff.id as id,diff.id_serv as id_ser, diff.id_doc as nom from diff where id_doc='.$r10.'');	
								}
								elseif($adp==3){
								$req1=$connexion->query('select diff.id as id,diff.id_serv as id_ser, diff.id_doc as nom from diff where id_doc='.$r10.'');	
								}
								...
								$SelectSserviceInfo=$connexion->prepare("select sservice.nom as nom ,sservice.id_service as id_service from sservice where sservice.id=?");
								$SelectSserviceInfo->execute(array($id_sser));
								$SelectDocumentInfo=$connexion->prepare("select document.nom as nom , document.type as type  from document where document.id=$id_doc");
								$SelectDocumentInfo->execute(array($id_doc));
								$objSservice = $SelectSserviceInfo->Fetch(PDO::FETCH_ASSOC);
								$objDocument = $SelectDocumentInfo->Fetch(PDO::FETCH_ASSOC);
								$Sservnom=$objSservice['nom'];
								$SservServID=$objSservice['id_service'];
								$type=$objDocument['type'];
								$nom=$objDocument['nom'];
									
								echo '<tr class="odd gradeX"><td>'.utf8_encode($objDocument['nom']).'</td>';
								echo '<td>'.$type.'</td>';
								
								
								echo '<td>'.utf8_encode(getNomService($SservServID)).'</td>';
								
								echo '<td>'.utf8_encode($Sservnom).'</td>';
								echo '<td><center><a href="deldiff.php?doc_id='.$id_diff.'&ser='.$_GET['ser'].'" onclick="return(confirm(\'Etes-vous sûr de vouloir annuler cette diffusion?\'))"; id="Acc" ><span class="fa fa-trash-o"></span></a></center></td>';
