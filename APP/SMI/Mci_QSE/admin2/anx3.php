					...
					<div class="tabbable tabbable-custom boxless tabbable-reversed">
						...
						<div class="tab-content">
							<div class="tab-pane active" id="tab_0">
								...
								<form action="anx3.php?file=<?php echo urlencode($_GET['file']) ; ?>&type=<?php echo $type;?>&ser=<?php 

echo $ser;?>&doca=0" class="form-horizontal" method="POST"">
											<div class="form-body">
											...
											<?php
											$i=0;
											$rv=addslashes($_GET['file']);
												$getid=$connexion->query("select id as id_doc from document where document.nom='".$rv."'");												
												$getiddoc = $getid->Fetch(PDO::FETCH_ASSOC);
												$id_doc= $getiddoc['id_doc'];												
												$asso=$connexion->query("select liaison.id_anx as anx from liaison where liaison.id_doc='$id_doc'");
												$nbrasso=$asso->rowcount();
											if($nbrasso<$var){	
													while($i<$nbrasso){
											while($liason = $asso->Fetch(PDO::FETCH_ASSOC)){																						
												$anx= $liason['anx'];
												$getnom=$connexion->query("select  nom as nom from document where document.id='$anx'");
												$nomdoc = $getnom->Fetch(PDO::FETCH_ASSOC);												
												$anxnom = $nomdoc['nom'];
												echo'<select name="doc[]" class="select2me form-control">';													
												echo'<option value="'.utf8_encode($anxnom).'">'.utf8_encode($anxnom).'</option>';
												$re=$connexion->query("SELECT nom as nom from document where 
												document.nom!='".$rv."' and document.Etat!='annule' and document.Etat!='perime' ");
										...
										while($i<$var){
													...
													echo'<select name="doc[]" class="select2me form-control">';
													echo'<option value="">Choix de document associé</option>';
													while($ose2 = $re->Fetch(PDO::FETCH_ASSOC)){
														$nom=$ose2['nom'];
														echo'<option value="'.utf8_encode($nom).'">'.utf8_encode($nom).'</option>';
														}		echo'</select></br>';
...
<?php
	if (isset($_POST['annule'])){
		...
		$ser=$_GET['ser'];
		$type=$_GET['type'];
		header('Location: docdoc.php?service='.$ser.'&type='.$type.'');
	}
	...
	$r3=$obj2['id'];
	$req3=$connexion->query("select liaison.id_anx as anx from liaison where liaison.id_doc='$r3'");
		$nbr3=$req3->rowcount();		
			while($obj=$req3->Fetch(PDO::FETCH_ASSOC)){
				$del = $connexion->query("DELETE FROM liaison WHERE liaison.id_doc=$r3");
						}
