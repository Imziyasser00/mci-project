							...
							<div class="tab-pane active" id="tab_2">
								<div class="portlet box green">
									...
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
										<form action="nmbrasso.php?id=<?php echo $_GET['id']; ?>" class="form-horizontal" method="post" enctype="multipart/form-data">
											<div class="form-body"
												...
												<div class="row">
													<div class="col-md-offset-3 col-md-9">
														...
														<button type="submit" name="annuler"  class="btn btn-circle default">Annuler</button>
													</div>
												</div>
													<div class="col-md-6">
													</div>
...
if (isset($_POST['annuler'])){
	include_once 'bd.php';
	$od=$_GET['id'];
		$req=$connexion->query("select nom, type , id_service from document where document.id=$od");
	$nom=$req->Fetch(PDO::FETCH_ASSOC);
		$type=$nom['type'];
	    $ser=$nom['id_service'];
	$req1=$connexion->query("select nom from service where id=$ser");
	$nom2=$req1->Fetch(PDO::FETCH_ASSOC);
	$ser=$nom2['nom'];
	...
	$req=$connexion->query("select nom, type , id_service from document where document.id=$od");
	$nom=$req->Fetch(PDO::FETCH_ASSOC);
	$file=utf8_encode($nom['nom']);
	$type=utf8_encode($nom['type']);
	$str=urlencode($file);
	$ser=$nom['id_service'];
if (isset($_POST['valider'])){
	...
	$nom2=$req1->Fetch(PDO::FETCH_ASSOC);
	$ser=$nom2['nom'];
	header("Location: anx3.php?file=$str&ser=$ser&type=$type&doca=$nda&t=1");
		}		
	else{
		
		$req3=$connexion->query("select liaison.id_anx as anx from liaison where liaison.id_doc='$od'");
		$nbr3=$req3->rowcount();		
			while($obj=$req3->Fetch(PDO::FETCH_ASSOC)){
				$del = $connexion->query("DELETE FROM liaison WHERE liaison.id_doc=$od");
						}
						$req1=$connexion->query("select nom from service where id=$ser");
	$nom2=$req1->Fetch(PDO::FETCH_ASSOC);
	$ser=$nom2['nom'];
