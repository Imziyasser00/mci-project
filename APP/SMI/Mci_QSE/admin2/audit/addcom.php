  ...
  ob_start(null, 0, false);
	...
	$num=$_GET['num'];	
		...
		header("Location: 404.html");	
			...
			$compfiche=$connexion->exec("INSERT INTO `fichecomqse` values ('',$id_fiche ,$reps1,1)");
			$creerapport=$connexion->exec("INSERT INTO `rapportcreeoupas` values ('','".$num."' ,0,'')");
			if($compfiche!=0){						
			...
			$confirmaudita=$connexion->query("select * from auditeurprevu where numero_audit='".$num."' and fonction='1'");
							...
							$to = $mail;
								...
								$txt = 'La fiche d\'audit n° '.$num.' a été validée par le responsable Management Q.S.E.
								<br> <a href="http://192.168.2.106/SMI/Mci_QSE/admin/pdf/reg/gerfiche.php?num='.$num.'">téléchargement</a>.
							<br>veuillez <a href="http://192.168.2.106/SMI/Mci_QSE/admin/login.php">vous connecter</a> pour rédiger le rapport d\'audit.
			<br><br>Merci de ne pas répondre à ce courriel!<br><br>SMI Q.S.E.<br>M.C.I Santé Animale ';	
								$headers =  "From: noreply@mci-santeanimale.com" . "\r\n";
								$headers .= "Content-Type: text/html; charset=\"iso-8859-1\"";
								mail($to,$subject,$txt,$headers);
