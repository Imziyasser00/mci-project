					...
					<div class="portlet box grey-cascade">
						...
						<div class="portlet-body">
							...
							<tbody>
								...
								$p=0;
								if($adp==2){
								$req1=$connexion->query("select docprovisoire.id as id,docprovisoire.id_service as id_ser, docprovisoire.nom as nom,docprovisoire.type as type,docprovisoire.cop as cop from docprovisoire where docprovisoire.ajtpar=2");
								}
								elseif($adp==3){
								$req1=$connexion->query("select docprovisoire.id as id,docprovisoire.id_service as id_ser, docprovisoire.nom as nom,docprovisoire.type as type,docprovisoire.cop as cop from docprovisoire where docprovisoire.ajtpar=3");
								}
								elseif($adp==1){
								$req1=$connexion->query("select docprovisoire.id as id,docprovisoire.id_service as id_ser, docprovisoire.nom as nom,docprovisoire.type as type,docprovisoire.cop as cop from docprovisoire");
								}
								$nbra=$req1->rowcount();
								if($nbra!=0){
								...
								while  ($obj_exp = $req1->Fetch(PDO::FETCH_ASSOC)) 
								{
									...
									elseif($cop=="2"){
										$cop='<span class="label label-sm label-warning">Révision</span>';
									}
									elseif($cop=="1"){
										$cop='<span class="label label-sm label-info">Créé</span>';
									}
									...
									$var='../Documents/'.$serv_exp.'/'.$type_exp.'/'.utf8_encode($nom_exp).'.pdf';			
								echo '<tr><td>'.$nom_exp.'</td>
									<td>'.$serv_exp.'</td>
									<td>'.$type_exp.'</td>									
								...
								$cop=$obj_exp['cop'];
									if($cop=="0"){
										echo'
									<td><a class="" href="creedoc.php?doc_id='.$id_doc_exp.'"id="Acc" title="Upload"><span class="fa fa-file-pdf-o"></span></a></td>
									</tr>';}																			
									else{									
										echo'<td><a href="'.$var.'" class="fancybox fancybox.iframe" title="'.$nom_exp.'"><span class="fa fa-eye"></span></a></td>
									</tr>';}
								}								
								}								
								else{
