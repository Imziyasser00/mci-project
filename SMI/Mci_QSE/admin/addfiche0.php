...
if (version_compare(PHP_VERSION, '5.4.0', '>=')) {
  ...
  ob_start(null, 0, false);
	...
	$num=$_GET['num'];	
		...
		$creeObjectif=$connexion->exec("INSERT INTO `Objectifaudit` values (null,$tab,'".$num."')");
		$cleerObjectif=$connexion->exec("DELETE FROM `Objectifaudit` WHERE objectif=''");
		}
			$creefiche=$connexion->prepare("INSERT INTO `fichdaudit` values (null,?,? ,?,?,?,?)");
			$creefiche->execute(array($id_RA,$id_ss,$date,$lieu,$idr,$num));
		
			if($creefiche!=0){
				while ($monchoix = array_shift($auditee)){				
				$result3=$connexion->exec("INSERT INTO auditee  VALUES (null, '".$num."', ".$monchoix." )");
	
			}
			$reponsable = $_POST['reponsable'];
			$creeREA=$connexion->exec("INSERT INTO `respentaudite` values (null,$reponsable ,'$num')");
