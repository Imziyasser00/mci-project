  ...
  ob_start(null, 0, false);
	...
	$num=$_GET['num'];	
		...
		if(isset($_POST['reunioncloture']) and $_POST['reunioncloture']!=''){
			...
			$creefiche=$connexion->exec("INSERT INTO `reunioncloture` values (null ,'".$date."' ,'".$time."','".$num."')");
			if($creefiche!=0){
				while ($monchoix = array_shift($reunioncloture)){				
				$result3=$connexion->exec("INSERT INTO `presencecloture`  VALUES (null, '$num', $monchoix )");
