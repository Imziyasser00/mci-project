							...
							<div class="tab-pane active" id="tab_0">
								<div class="portlet box green">
									...
									<div class="portlet-body form">
										<!-- BEGIN FORM-->
										<form action="addservice.php" class="form-horizontal" method='POST'>
											<div class="form-body">
												<div class="form-group">
													<label class="col-md-3 control-label">Nom du service</label>
													<div class="col-md-4">
														<div class="input-group">
															<input type="text" name="nom" class="form-control input-circle-left" placeholder="">
												...
												<div class="form-group">
													<label class="col-md-3 control-label">Réfèrence du service</label>
													<div class="col-md-4">
														<div class="input-icon right">															
															<input type="text" name="ref" class="form-control input-circle" placeholder="Réfèrence">
														</div>
													</div>
												</div>												
...
if(isset($_POST['valider'])){
	if(isset($_POST['nom']) and isset($_POST['ref'])){
		if(($_POST['nom'])!=NULL and ($_POST['ref'])!=NULL){
			include_once 'bd.php';
			$nom=$_POST['nom'];
			$ref=$_POST['ref'];
			$verifier=$connexion->query("SELECT * from service where service.nom='$nom' or service.ref='$ref'");
			$nbr=$verifier->rowcount();
			$copy="../Documents/index.php";
				if($nbr == 0){					
				mkdir('../Documents/'.$nom.'', 0777);
				mkdir('../Documents/'.$nom.'/FAP/', 0777);				
				mkdir('../Documents/'.$nom.'/FAP/Archive/', 0777);				
				$selecttype=$connexion->query("select type as nom from type");				
					while($type=$selecttype->Fetch(PDO::FETCH_ASSOC)){
					$nomtype=$type['nom'];
					mkdir('../Documents/'.$nom.'/'.$nomtype.'', 0777);					
					mkdir('../Documents/'.$nom.'/'.$nomtype.'/Archive/', 0777);					
					$to2='../Documents/'.$nom.'/'.$nomtype.'/index.php';
					$to4='../Documents/'.$nom.'/'.$nomtype.'/Archive/index.php';
					copy($copy,$to2);
					copy($copy,$to4);
					}
					$to='../Documents/'.$nom.'/index.php';
					$to3='../Documents/'.$nom.'/FAP/index.php';
					$to5='../Documents/'.$nom.'/FAP/Archive/index.php';
					copy($copy,$to);
					copy($copy,$to3);
					copy($copy,$to5);
				$insertservice=$connexion->exec("insert into service values(null,'$nom','$ref')");
					if($insertservice!=NULL){
						header("location: listeservice.php");
					}			
					else{
						echo '<script language="javascript" type="text/javascript">
						alert("ERREUR SYSTEME : 707");</script>';
					}
				}
				else{
				echo '<script language="javascript" type="text/javascript">
       alert("Le nom ou bien la réfèrence du service déjà éxistante ");</script>';
		...
		echo '<script language="javascript" type="text/javascript">
       alert("Veuiller insérer le nom et la réfèrence du service avant de valider");</script>';
		}
	}
}
}
