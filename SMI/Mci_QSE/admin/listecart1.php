...
if($id==43 || $id==34|| $id==31 || $id==38 || $id==32 || $id==42 || $id==39 || $id==37 || $id==41 || $id==29){
	return 2;
}
...
function annNum($ann,$idp){
    ...
    $cnx=$connexion->prepare('select numero from auditprevu where  ann=? and id_sservice=?');
   $cnx->execute(array($ann,$idp));
   $rowcount=$cnx->rowcount();
  if($rowcount!=0){
       $obj = $cnx->Fetch(PDO::FETCH_ASSOC);
      return $obj['numero'];
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-file-o"></i>Liste des Ecarts service : <?php echo getNomSservice($idproc); ?>
							</div>
							<div class="tools">
						...
						<div class="portlet-body">
							...
							<tbody>
								...
								<?php			
								include_once 'bd.php';
								$nums =annNum($an,$idproc);
								$selectEcmaj=$connexion->query("select * from em where annee = '".$an."' and num_audit='".$nums."' and etat=0");
								$nEcmaj=$selectEcmaj->rowcount();								
								if($nEcmaj!=0){
								$commentaire="";
								while  ($obj = $selectEcmaj->Fetch(PDO::FETCH_ASSOC)) 
								{
									$idpoint=$obj['id_point'];
									$nume=$obj['num_audit'];
								$selectEcmaj2=$connexion->query('select * from point where id='.$idpoint.' and commentaire!=" "');	
								...
								echo '<tr>
								<td>'.sservice(getNomSservice($idproc)).'</td>
								<td>'.$nume.'</td>
								<td>'.$commentaire.'</td>
								<td>
								<a class="" href="suppem.php?idproc='.$idproc.'&idpoint='.$idpoint.'&an='.$an.'" onclick="return(confirm(\'Etes-vous sûr de vouloir supprimer ?\'))"; id="Acc" title="supprimer">      <span class="fa fa-trash-o"></span></a>
