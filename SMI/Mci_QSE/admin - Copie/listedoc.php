					...
					<div class="portlet box grey-cascade">
						...
						<div class="portlet-body">
							...
							<tbody>
								...
								include_once 'bd.php';
								if($adp==2){
								$req1=$connexion->query("select document.id as id,document.id_service as id_ser, document.nom as nom, document.lien as lien,document.dateAPP as app,document.DATTREV as rev, document.Etat as etat, document.type as type from document where document.ajtpar=2 and document.Etat != 'Annule'  and  document.Etat != 'Perime'");
								}
								elseif($adp==3){
								$req1=$connexion->query("select document.id as id,document.id_service as id_ser, document.nom as nom, document.lien as lien,document.dateAPP as app,document.DATTREV as rev, document.Etat as etat, document.type as type from document where document.ajtpar=3 and document.Etat != 'Annule'  and  document.Etat != 'Perime'");
								}
								elseif($adp==1){
								$req1=$connexion->query("select document.id as id,document.id_service as id_ser, document.nom as nom, document.lien as lien,document.dateAPP as app,document.DATTREV as rev, document.Etat as etat, document.type as type from document where document.Etat != 'Annule' and  document.Etat != 'Perime'");	
								}
								$nbra=$req1->rowcount();
								if($nbra!=0){
								...
								$expire = date("Ymd", strtotime($rev));
								echo '<tr class="odd gradeX"><td>'.utf8_encode($serv).'</td>';
								echo '<td>'.$type.'</td>';
								if($rev!="2099-10-10"){
								if($expire < $now) {
								$etat="Périmé";
								$rt3=$connexion->quote(utf8_decode($etat));	
								$reqmod=$connexion->exec("UPDATE document  SET document.Etat=$rt3 where document.id=$id_doc ");
								}
								}
								...
								echo '<td>'.utf8_encode($modnom).'<a class="" href="moddoc.php?doc_id='.$id_doc.'" onclick="return(confirm(\'Etes-vous sûr de vouloir Modifier ce document?\'))"; id="Acc" title="Modifier">      <span class="fa fa-pencil"></span></a></td>';
							...
							elseif($etat=="Reconduit"){
								...
								echo '<td>'.$rev.'</td>';
									...
									if($rev!="2099-10-10"){
									$rev = date("d/m/Y", strtotime($rev));
									 echo '<td><center><span class="label label-sm label-danger">'.$rev.' </span></center></td>';	
									}
									else{
										$rev = "jusqu'à modification";
								echo '<td>'.$rev.'</td>';
									}
								}
								$var='../Documents/'.$serv.'/'.$type.'/'.$nom.'';
								
								echo  '          <td> <a href="'.$var.'" class="fancybox fancybox.iframe" title="'.$modnom.'"><span class="fa fa-eye"></span></a>
								...
								if($nbr3==0){
									...
									echo '	<a href="liste.php?id_doc='.$id_doc.'" class="fancybox fancybox.iframe" title="Liste des documents associés"><span class="fa fa-chain "></span></a>											
