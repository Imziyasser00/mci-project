...
<html lang="fr"> 
 <?php
	...
	$reqa2=$connexion->query("select id from service");
	$nbrcount2=$reqa2->rowcount();
	$adp=$_SESSION['adp'];
	$openfrom=0;
?>
<head>
				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							<div class="caption">
								<i class="fa fa-wrench"></i>Listes des Services
							</div>
							<div class="tools">
							</div>
						</div>
						...
						<?php			
								include_once 'bd.php';
								$req1=$connexion->query("select service.id as id,service.ref as ref, service.nom as nom from service where 1");
								$nbr=$req1->rowcount();								
								if($nbr!=0){								
								while  ($obj = $req1->Fetch(PDO::FETCH_ASSOC)) 
								{	
							$idd=$obj['id'];
										$verif=$connexion->query("Select * from document where id_service=$idd ");
											$nbr2=$verif->rowcount();
												if($nbr2!=0){
												$dire='href="#" onclick="alert(\'Impossible de supprimer ce service \')";';
												}
												else{
												$dire='href="delserv.php?id='.$idd.'" onclick="return(confirm(\'Etes-vous sûr de vouloir supprimer ce service ?\'))";'	;
												}
											
											
											
											
								echo ' <tr><td>'.$obj['id'].'</td><td>'.$obj["ref"].'</td><td>'.$obj["nom"].'</td>								
								<td><a '.$dire.' id="Acc" title="Supprimer">Supprimer</a>
